...
import { calculateWinRate, calculateOverallReputation, type AgentStats } from '@/lib/agentApi';
...
export function AgentStatsCard({ agent }: AgentStatsCardProps) {
  ...
  const stats: Array<{ label: string; value: string; icon: React.ReactNode; highlight?: boolean; subtext?: string }> = [
    {
      label: 'Markets',
    ...
    {
      label: 'Net P&L',
      value: `${((Number(agent.totalWinnings) - Number(agent.totalStaked)) / 1e6).toFixed(6)} USDC`,
      icon: (
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    {
      label: 'Reputation',
      value: `${calculateOverallReputation(agent).toFixed(1)}%`,
      subtext: 'confidence + winrate',
      icon: (
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-chainlink-surface rounded-xl p-3">
            ...
            {stat.subtext && (
              <div className="text-[10px] text-chainlink-text-muted mt-0.5">{stat.subtext}</div>
            )}
          </div>
        ))}
      </div>
