...
init_toHex();
init_keccak256();
var drandNetworkSchema = exports_external.object({
  chainHash: exports_external.string().startsWith("0x"),
  genesis: exports_external.number().positive(),
  period: exports_external.number().positive(),
  httpClient: exports_external.string().min(1)
});
var evmConfigSchema = exports_external.object({
  chainSelectorName: exports_external.string().min(1),
  marketAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/u, "marketAddress must be a valid address"),
  gasLimit: exports_external.string().regex(/^\d+$/, "gasLimit must be a numeric string")
});
...
var GeminiResponseSchema = exports_external.object({
  result: exports_external.enum(["YES", "NO", "INCONCLUSIVE"]),
...
`;
var userPrompt = `Determine the outcome of this market based on factual information and return the result in this JSON format:
...
Market question:
`;
var askGemini = (runtime2, marketId, question, _schemaURI) => {
  let geminiApiKey;
  const secret = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
  geminiApiKey = secret.value;
  const httpClient = new cre.capabilities.HTTPClient;
  const result = httpClient.sendRequest(runtime2, postGeminiData({ marketId, question }, geminiApiKey), consensusIdenticalAggregation())(runtime2.config).result();
  return result;
};
var postGeminiData = (logDetails, geminiApiKey) => (sendRequester, config) => {
  const dataToSend = {
...
var fetchSchema = (runtime2, schemaURI) => {
  const httpClient = new cre.capabilities.HTTPClient;
  const result = httpClient.sendRequest(runtime2, fetchSchemaRequest(schemaURI), consensusIdenticalAggregation())(runtime2.config).result();
  return result;
};
var fetchSchemaRequest = (schemaURI) => (sendRequester, _config) => {
  let url;
...
var MINI_MARKET_ABI = JSON.parse(readFileSync(resolve("./abis/MiniMarket.json"), "utf-8"));
var INFO_REVEAL_HASH = keccak256(toHex("InfoRevealRequested(uint256,uint64,uint256)"));
var RESOLUTION_HASH = keccak256(toHex("ResolutionRequested(uint256,string,uint48)"));
var onResolutionTrigger = (runtime2, log) => {
  runtime2.log("ResolutionRequested event received");
  ...
  runtime2.log(`Market ID: ${marketId}`);
  runtime2.log(`Schema URI: ${schemaURI}`);
  try {
    const schemaResult = fetchSchema(runtime2, schemaURI);
    runtime2.log(`Schema fetched: ${schemaResult.schema.type}`);
    const deadline = schemaResult.schema.deadline;
    const currentTimestamp = Math.floor(Date.now() / 1000);
    if (currentTimestamp < deadline) {
      runtime2.log(`Deadline not reached (${deadline}), skipping resolution`);
      return "SKIPPED_NOT_YET";
    }
    const geminiResponse = askGemini(runtime2, marketId.toString(), schemaResult.schema.description || "Resolve this market");
    ...
    runtime2.log(`Resolution: ${result}`);
    runtime2.log(`Confidence: ${confidence}`);
    return `RESOLVED_${result}`;
  } catch (error) {
    runtime2.log(`Error processing resolution: ${error}`);
    throw error;
  }
};
var initWorkflow = (config) => {
  ...
  return [
    cre.handler(evmClient.logTrigger(triggerConfig), onResolutionTrigger)
  ];
};
async function main() {
