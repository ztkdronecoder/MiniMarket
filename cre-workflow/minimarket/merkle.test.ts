...
describe("Merkle Utilities", () => {
  ...
  describe("buildMerkleTree", () => {
    it("should build tree with single leaf", () => {
      ...
      expect(verifyProof(root, leafHashes[0], proof, 0)).toBe(true);
    });
    ...
    it("should build tree with two leaves and generate valid proofs", () => {
      ...
      expect(verifyProof(root, leafHashes[0], proof0, 0)).toBe(true);
      expect(verifyProof(root, leafHashes[1], proof1, 1)).toBe(true);
    });
    ...
    it("should build tree with four leaves", () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        expect(verifyProof(root, leafHashes[i], proof, i)).toBe(true);
      }
    });
  ...
  describe("verifyProof", () => {
    it("should reject invalid proof", () => {
      ...
      expect(verifyProof(root, wrongLeafHash, proof, 0)).toBe(false);
    });
    ...
    it("should reject wrong index", () => {
      ...
      expect(verifyProof(root, leafHashes[0], proof1, 0)).toBe(false);
    });
    ...
    it("should reject proof from different tree", () => {
      ...
      expect(verifyProof(tree2.root, tree1.leaves[0], proof1, 0)).toBe(false);
    });
  });
  ...
  describe("computeValidationHash", () => {
    it("should compute deterministic hash", () => {
      ...
      const yesPercent = BigInt(7000);
      const noPercent = BigInt(3000);
    ...
    it("should produce different hashes for different salts", () => {
      const agent = "0x1234567890123456789012345678901234567890" as `0x${string}`;
      const yesPercent = BigInt(5000);
      const noPercent = BigInt(5000);
    ...
    it("should produce different hashes for different percentages", () => {
      ...
      const hash1 = computeValidationHash(agent, BigInt(7000), BigInt(3000), salt);
      const hash2 = computeValidationHash(agent, BigInt(5000), BigInt(5000), salt);
  ...
  describe("cross-validation with contract", () => {
    it("should match contract merkle tree for two leaves", () => {
      ...
      expect(verifyProof(root, leafHashes[0], proof1, 0)).toBe(false);
      expect(verifyProof(root, leafHashes[1], proof0, 1)).toBe(false);
    });
  });
});
