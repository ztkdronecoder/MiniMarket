...
var evmConfigSchema = exports_external.object({
  ...
  marketAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/u),
  gasLimit: exports_external.string().regex(/^\d+$/)
});
var configSchema = exports_external.object({
  schedule: exports_external.string().default("0 */2 * * * *"),
  ponderUrl: exports_external.string().min(1).default("http://127.0.0.1:42069"),
  evms: exports_external.array(evmConfigSchema).min(1)
});
var resolutionSchemaType = exports_external.enum(["price", "sports", "weather", "ai", "custom"]);
var resolutionSourceSchema = exports_external.object({
  type: exports_external.enum(["api", "chainlink", "web"]),
  url: exports_external.string().optional(),
  jsonPath: exports_external.string().optional(),
  transform: exports_external.string().optional(),
  address: exports_external.string().optional(),
  chain: exports_external.string().optional()
});
var resolutionSchema = exports_external.object({
  ...
  resolution: exports_external.object({
    ...
    sources: exports_external.array(resolutionSourceSchema),
    comparator: exports_external.enum([">", "<", "==", ">=", "<="]).optional(),
    targetValue: exports_external.string()
  }),
  fallback: exports_external.object({
    type: exports_external.literal("ai"),
    provider: exports_external.literal("gemini"),
    prompt: exports_external.string()
  }).optional()
});
var MODEL = "gemini-2.5-flash";
var systemInstruction = `You are a binary prediction market resolver. Your job is to determine the real-world outcome of market conditions using Google Search.
` + `
...
var askGemini = (runtime2, marketId, question, submarkets) => {
  const secret = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
  const geminiApiKey = secret.value;
  const httpClient = new cre.capabilities.HTTPClient;
  return httpClient.sendRequest(runtime2, postGeminiData({ marketId, question, submarkets }, geminiApiKey), consensusIdenticalAggregation())(runtime2.config).result();
};
var postGeminiData = (logDetails, geminiApiKey) => (sendRequester, _config) => {
  const submarketLines = logDetails.submarkets.length > 0 ? logDetails.submarkets.map((s) => `Submarket-${s.optionIndex}: ${s.optionLabel?.trim() || logDetails.question}`).join(`
`) : `Submarket-0: ${logDetails.question}`;
  const userPrompt = `Market question: "${logDetails.question}"

` + submarketLines + `

Use Google Search to find the real-world outcome for each submarket. ` + `Respond with ONLY the JSON array — no other text.`;
  const body = {
  ...
  const textParts = parts.filter((p) => typeof p.text === "string" && !(("thought" in p) && p.thought)).map((p) => p.text);
...
var fetchPhase2PendingMarkets = (runtime2) => {
  let result;
  try {
    const httpClient = new cre.capabilities.HTTPClient;
    result = httpClient.sendRequest(runtime2, fetchNextPhase2Request(runtime2.config.ponderUrl), consensusIdenticalAggregation())(runtime2.config).result();
  } catch (e) {
    runtime2.log(`Ponder next-phase2 fetch failed: ${e}`);
    return [];
  }
  return result;
};
var fetchNextPhase2Request = (baseUrl) => (sendRequester, _config) => {
  const req = {
...
var onCronTrigger = (runtime2) => {
  runtime2.log("Phase 2 cron triggered");
  const pending = fetchPhase2PendingMarkets(runtime2);
  if (pending.length === 0) {
    runtime2.log("No phase2 markets to resolve");
    return "NO_PENDING";
  }
  ...
  if (submarkets.length === 0) {
    runtime2.log(`Market ${marketId}: no submarkets to resolve`);
    return "SKIPPED_NO_SUBMARKETS";
  }
  try {
    let question = market.question ?? "";
    if (market.schema && typeof market.schema === "object") {
      const s = market.schema;
      question = s.description ?? s.resolution?.prompt ?? s.fallback?.prompt ?? question;
    }
    runtime2.log(`Market ${marketId}: asking Gemini for ${submarkets.length} submarket(s)`);
    const geminiResponse = askGemini(runtime2, market.marketId, question, submarkets);
    runtime2.log(`Gemini response: ${geminiResponse.geminiResponse}`);
    let parsed;
    try {
      parsed = JSON.parse(geminiResponse.geminiResponse);
    } catch {
      runtime2.log("Invalid JSON from Gemini");
      return "SKIPPED_INVALID_RESPONSE";
    }
    const resolutions = parsed.resolutions ?? [];
    if (resolutions.length === 0 || (parsed.confidence ?? 0) === 0) {
      runtime2.log(`Market ${marketId}: inconclusive`);
    ...
    for (const sm of submarkets) {
      ...
      if (!res || res.outcome !== "YES" && res.outcome !== "NO") {
        runtime2.log(`Market ${marketId}: missing resolution for submarket ${sm.optionIndex}`);
        return "SKIPPED_INCOMPLETE_RESOLUTION";
      }
      const sid = sm.submarketId.startsWith("0x") ? sm.submarketId : `0x${sm.submarketId}`;
      smIds.push(sid.toLowerCase());
      outcomes.push(res.outcome === "YES" ? 1 : 2);
    }
    runtime2.log(`Resolving ${smIds.length} submarket(s): ${outcomes.map((o) => o === 1 ? "YES" : "NO").join(", ")}`);
    const reportData = encodeAbiParameters(parseAbiParameters("uint8, bytes32[], uint8[]"), [3, smIds, outcomes]);
    ...
    if (!networkInfo) {
      runtime2.log("Network not found");
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`Resolved market ${marketId} (${smIds.length} submarkets): ${txHash}`);
